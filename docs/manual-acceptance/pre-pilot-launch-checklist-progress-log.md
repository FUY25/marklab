...
| 6 | Login, onboarding, workspace UI | Passed | TBD | Passed for the small controlled pilot. Hosted Google/OIDC owner login, native callback handling, minimal workspace select/create, minimal account settings sign-in/sign-out, app sign-in prompts, guest browser links, shared-state restore across app relaunch, and restart-sharing conflict suppression were verified on the installed app at commit `a44f19d8a7857e3d4806e4fc1ffb2fa8fa89d966`. Full product onboarding, workspace management, settings, and team-invite flows remain post-pilot/broader-beta work tracked below. Token-refresh/session-expiry hardening belongs to Gate 7; quota/provider-unavailable polish remains non-blocking for this pilot. |
...
| 2026-05-28 | Retained-cloud restart false-conflict flicker fixed during the shared-file startup performance pass. Root cause: after Stop Sharing then Start Sharing, the native app re-enabled the retained cloud copy and a file-watcher pass could queue a second disk ingestion while the provider was still hydrating from empty state; this briefly opened Conflict Review before the real provider snapshot cleared it. The fix restores the retained baseline before watching, skips disk ingestion when local markdown matches the retained baseline, and prevents file-watcher ingestion from replacing an already pending disk ingestion. | User manual retest passed; targeted retained-cloud restart tests added for unchanged local restart and pending-ingestion watcher suppression; `swift test --package-path apps/marklab-macos` returned 107 passed; `git diff --check`; `package:app`; `verify:package`. Temporary sync-debug file logging used for diagnosis was removed from source after confirmation. | Keep the fix; continue with pilot-product backlog triage below. |
...
Post-Gate 6 product gaps to track before broader beta, public launch, or paid launch:

- [ ] Full onboarding flow: first-run education, guided "open a Markdown file -> sign in -> Start Sharing -> copy link" path, empty-state guidance beyond the current clickable file opener, and recovery copy when auth/workspace/share setup fails.
- [ ] Workspace management page: workspace rename/switching, member list, roles, owner transfer or explicit non-support, workspace-level document/access overview, and clear retained-cloud-copy implications.
- [ ] Team invite flow: owner-created invite link/email, accept invite after Google sign-up/login, membership creation in the owner workspace, invite expiry/revocation, and role enforcement.
- [ ] Full settings page: profile/account state, sign out, local autosave, update channel/status, privacy/storage controls, support/diagnostic export, and destructive action placement. Current Settings only covers the minimum pilot account/local-autosave surface.
- [ ] Multi-workspace behavior: active workspace selection in native app and hosted web, clear default workspace, and deterministic behavior when an OIDC user belongs to multiple workspaces.
- [ ] Workspace/account deletion and data export: explicit owner-facing copy, retention behavior, and support workflow. Hard delete remains deferred; do not imply self-serve deletion yet.
- [ ] Plan/quota UI: show free/alpha limits, quota exceeded states, and upgrade/billing entry points only after Gate 11 pricing and no-loss floor are decided.
- [ ] Support/admin visibility: pilot support view or runbook for seeing a user's workspace, active shared documents, recent errors, and revoke/delete actions without direct DB spelunking.
...
Triage the Post-Gate 6 product gaps above into small-pilot blockers vs broader-beta requirements, then decide whether to start Gate 9 pilot prep or continue the targeted startup-performance/code-simplification pass.

Next acceptance row: if continuing performance work, measure Start Sharing and already-shared file reopen latency after the retained-cloud restart fix and identify any remaining blocking steps before broader active-code simplification.
