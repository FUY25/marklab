...
Plan 4 refresh note: native app session spoofing is now covered by the control-plane route contract. The API only preserves `clientKind: "app"` when the edit-session request carries a MarkLab user bearer token (`ml_user_...`), `X-MarkLab-Native-App: 1`, and resolves to a non-guest actor. Public links, guest sessions, and cookie-authenticated browser tabs are downgraded to `browser`, and the embedded native collab-web client treats that downgrade as unavailable instead of silently continuing with wrong audit metadata. Future control-plane changes must keep this server-side derivation rule and must not trust client-supplied actor ids or client-supplied app kind.
