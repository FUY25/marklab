...
27. Plan 6.2's browser-visible API write smoke requires a real Web remote document route. That work is now explicitly owned by `plans/06_3_web_remote_document_mode_plan.md`; Plan 7 CLI/skill work does not cover browser product E2E.
28. `plans/06_4_web_document_lifecycle_ui_plan.md` owns Web create/import/open/export controls. Backend import/export routes alone are not enough for MVP product testing.
29. `plans/06_5_web_version_branch_ui_plan.md` owns Web version history, branch switching, branch-from-version, and restore-as-new-version UI. Plan 5 only provides backend primitives.
30. `plans/06_6_access_tokens_share_links_plan.md` owns document-scoped share links, agent token creation/revocation, controlled-MVP admin/bootstrap token, production auth mode, and Hocuspocus access checks. The initial schema tables are not sufficient for deployment without these routes and checks.
31. `plans/08_1_deployment_hardening_plan.md` extends the baseline deployment plan with schema application, web image/config, readiness checks, smoke scripts, and runbook. `plans/11_mvp_launch_readiness_plan.md` is the final cross-plan acceptance gate.
