...
describe('createCollabServer persistence hooks', () => {
  ...
  it('applies local-store ingested state without re-entering local persistence', async () => {
    const initialState = createState('base');
    const providerState = updateState(initialState, (text) => text.insert(text.length, ' provider'));
    const localDiskState = updateState(providerState, (text) => {
      text.delete(0, text.length);
      text.insert(0, 'local disk');
    });
    ...
    const localStore = {
      ...
      storeRoomState: vi.fn(async () => ({
        stored: true,
        stateFingerprint: encodeYjsStateFingerprint(localDiskState),
        yjsState: localDiskState,
        prepare,
        commit,
      })),
    };
    ...
    try {
      const loadedState = await collab.server.configuration.onLoadDocument({
        documentName: 'local:file:abc123',
        document,
      });
      expect(loadedState).toBeInstanceOf(Uint8Array);
      ...
      const localApplyOrigins: unknown[] = [];
      document.on('update', (_update, origin) => {
        localApplyOrigins.push(origin);
        void collab.server.configuration.onStoreDocument({
          documentName: 'local:file:abc123',
          document,
        });
      });

      await collab.server.configuration.onStoreDocument({
        documentName: 'local:file:abc123',
        document,
      });
      await Promise.resolve();

      expect(textFromState(Y.encodeStateAsUpdate(document))).toBe('local disk');
      expect(localApplyOrigins).toHaveLength(1);
      expect(localStore.storeRoomState).toHaveBeenCalledTimes(1);
  ...
  it('does not commit a local-store ingested state when applying it to the provider document fails', async () => {
    const initialState = createState('base');
    const providerState = updateState(initialState, (text) => text.insert(text.length, ' provider'));
    const invalidState = new Uint8Array([1, 2, 3]);
    const prepare = vi.fn(async () => undefined);
    const commit = vi.fn(async () => undefined);
    const store = createPersistencePool(initialState);
    const localStore = {
      ...
      storeRoomState: vi.fn(async () => ({
        stored: true,
        stateFingerprint: encodeYjsStateFingerprint(invalidState),
        yjsState: invalidState,
        prepare,
        commit,
      })),
    };
    ...
    try {
      ...
      await expect(
        collab.server.configuration.onStoreDocument({
          documentName: 'local:file:abc123',
          document,
        }),
      ).rejects.toThrow();
  ...
  it('records a pending local-store ingestion before provider apply when local commit later fails', async () => {
    const initialState = createState('base');
    const providerState = updateState(initialState, (text) => text.insert(text.length, ' provider'));
    const localDiskState = updateState(providerState, (text) => {
      text.delete(0, text.length);
    ...
    const commit = vi.fn(async () => {
      throw new Error('metadata_commit_failed');
    });
    const store = createPersistencePool(initialState);
    const localStore = {
      canHandleRoom: (roomName: string) => roomName === 'local:file:abc123',
    ...
    try {
      ...
      ).rejects.toThrow('metadata_commit_failed');
  ...
  it('does not apply a local-store ingestion if provider state changes while the pending state is prepared', async () => {
    const initialState = createState('base');
    const providerState = updateState(initialState, (text) => text.insert(text.length, ' provider'));
    const remoteRaceState = updateState(providerState, (text) => text.insert(text.length, ' remote'));
    const localDiskState = updateState(providerState, (text) => {
      text.delete(0, text.length);
      text.insert(0, 'local disk');
    });
    ...
    const prepare = vi.fn(async () => {
      Y.applyUpdate(document, remoteRaceState);
    });
    ...
    try {
      ...
      expect(textFromState(Y.encodeStateAsUpdate(document))).toBe('base provider remote');
      expect(prepare).toHaveBeenCalledTimes(1);
  ...
  it('does not commit local-store metadata if provider state changes after local apply', async () => {
    const initialState = createState('base');
    const providerState = updateState(initialState, (text) => text.insert(text.length, ' provider'));
    const remoteRaceState = updateState(providerState, (text) => text.insert(text.length, ' remote'));
    const localDiskState = updateState(providerState, (text) => {
  ...
  it('does not advance loaded state when local-store commit reports failure', async () => {
    const initialState = createState('base');
    const providerState = updateState(initialState, (text) => text.insert(text.length, ' provider'));
    const localDiskState = updateState(providerState, (text) => {
      text.delete(0, text.length);
    ...
    const store = createPersistencePool(initialState);
    const document = new Y.Doc();
    const commit = vi.fn(async () => false as never);
    const localStore = {
      canHandleRoom: (roomName: string) => roomName === 'local:file:abc123',
      loadRoomState: vi.fn(async () => ({
        yjsState: initialState,
    ...
    try {
      ...
      expect(localStore.storeRoomState).toHaveBeenNthCalledWith(
        2,
        'local:file:abc123',
        expect.any(Uint8Array),
        encodeYjsStateFingerprint(initialState),
      );
    } finally {
      document.destroy();
      await collab.server.destroy?.();
  ...
  it('stops the local-store path when the local store reports sync is paused', async () => {
    const initialState = createState('base');
    const providerState = updateState(initialState, (text) => text.insert(text.length, ' provider'));
    const store = createPersistencePool(initialState);
    const localStore = {
