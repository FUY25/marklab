...
As of this alpha, npm installs `@marklab/cli@0.1.0-alpha.3` from the `latest` tag.
...
Yes: a collaborator can install the CLI, create the local file, join the shared edit link, and stay listening with one command.

```sh
npx -y @marklab/cli join '<edit-link>' --dir ./docs --name README.md --create-dir
```

Replace `<edit-link>` with the relay edit link from the host.

This command:

- Installs and runs the CLI through `npx`.
- Creates `./docs` if it does not exist.
- Creates `./docs/README.md` from the shared document.
- Starts a local MarkLab daemon for that mirror file.
- Keeps the terminal attached so the mirror keeps syncing.

Leave the terminal open. Press `Ctrl-C` when you want to stop listening.

If `./docs/README.md` already exists and is non-empty, MarkLab rejects the join instead of overwriting it. To intentionally replace it:

```sh
npx -y @marklab/cli join '<edit-link>' --dir ./docs --name README.md --create-dir --replace
```
