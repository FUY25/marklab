...
- Native local state uses the hosted `/collab` control-plane/Y-Sweet path. The old CLI daemon boundary and local app-context routes have been removed from the active package, so production/package smokes should prove the packaged CLI opens MarkLab.app, passes hosted workspace config through the native request bridge, and never depends on removed local API routes.
